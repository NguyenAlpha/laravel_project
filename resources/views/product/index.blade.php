...
@section('content')
  <div class="main">
    ...
    <div class="category-filter__box">
        ...
        @if(!empty($filters))
            <form action="{{ route('product.indexByCategory', ['category' => $category->category_id]) }}" 
                  method="get" 
                  id="filterForm" 
                  class="category-filter__form">
                  
                <!--
                    Trong Laravel, $filters đã có cấu trúc:
                    [
                        'thuong_hieu' => [
                            'label' => 'Thương hiệu',
                            'values' => ['HP', 'Asus', 'Dell'],
                ...
                <div class="category-filter__box__attribute">
                    ...
                    <select name="sapXep" class="form-select">
                        ...
                        <option value="gia-thap-den-cao" {{ request('sapXep') == 'gia-thap-den-cao' ? 'selected' : '' }}>Giá thấp đến cao</option>
                        <option value="gia-cao-den-thap" {{ request('sapXep') == 'gia-cao-den-thap' ? 'selected' : '' }}>Giá cao đến thấp</option>
                ...
                <div class="category-filter__box__attribute">
                    ...
                    <div class="category-price-inputs">
                        <input type="number" 
                               name="giaThap" 
                               placeholder="₫ từ" 
                               value="{{ request('giaThap') }}"
                               min="0">
                        <div class="category-price-inputs__bar"></div>
                        <input type="number" 
                               name="giaCao" 
                               placeholder="₫ đến" 
                               value="{{ request('giaCao') }}"
                               min="0">
                    </div>
                </div>
            ...
            @foreach ($products as $product)
              <div class="product__item__card">
                ...
                <div class="button__addcart__box">
                  <a href="">
                    <button class="button button__addcart" type="submit" name="addcart">Mua ngay</button></a>
                </div>
              </div>
            @endforeach
          </div>
